...
export async function runActorSync(actor, input, token, { timeoutMs = 240000 } = {}) {
  ...
  const url = `${APIFY_API}/acts/${encodeURIComponent(actor)}/run-sync-get-dataset-items?token=${encodeURIComponent(token)}&timeout=${timeoutMs / 1000}`;
  ...
  if (!res.ok) {
    // `status` is attached so the provider layer can classify this failure
    // (429 → rate limit, 403 → auth, 5xx → network) instead of string-matching.
